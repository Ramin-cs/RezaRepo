#!/usr/bin/env python3
"""
Perfect XSS Scanner - Ultimate Version
Persistent browser + Perfect popup screenshot + Safe exit
Author: AI Assistant
Version: 13.0 Perfect
"""
...
class PerfectXSSScanner:
    ...
    def calculate_professional_score(self, vuln_data):
        ...
        # Screenshot confirmation bonus (5 points)
    ...
    def phase2_persistent_browser_validation(self, recon_data):
        """Persistent browser validation - Chrome stays open throughout Phase 2"""
        self.log("=" * 80, "PHASE")
        self.log("PHASE 2: PERSISTENT BROWSER VALIDATION", "PHASE")
        self.log("=" * 80, "PHASE")
        ...
        try:
            ...
            self._test_all_parameters_persistent(recon_data)
        finally:
            self._close_browser()
    ...
    def _test_all_parameters_persistent(self, recon_data):
        """Test parameters with persistent browser"""
        # Test URL parameters
        for url in recon_data['urls']:
            if not self.running:
                break
            self._test_url_parameters_persistent(url)
        
        # Test form parameters
        for form in recon_data['forms']:
            if not self.running:
                break
            self._test_form_parameters_persistent(form)
    
    def _test_url_parameters_persistent(self, url):
        """Test URL parameters with persistent browser"""
        parsed_url = urlparse(url)
        ...
        for param_name in url_params:
            ...
            for base_payload in base_payloads:
                ...
                for payload in bypass_payloads:
                    ...
                    success = self._validate_xss_persistent_browser(url, None, payload, param_name, waf_types)
                    if success:
                        break
    ...
    def _test_form_parameters_persistent(self, form):
        """Test form parameters with persistent browser"""
        for input_field in form['inputs']:
            if not self.running:
                break
            if input_field['name']:
                ...
                for base_payload in base_payloads:
                    ...
                    for payload in bypass_payloads:
                        ...
                        success = self._validate_xss_persistent_browser(form['action'], form_data, payload, input_field['name'], waf_types)
                        if success:
                            break
    ...
    def _validate_xss_persistent_browser(self, url, form_data, payload, param_name, waf_types):
        """Persistent browser validation with perfect popup screenshot"""
        try:
            ...
            def handle_dialog(dialog):
                nonlocal alert_triggered, alert_message, alert_timestamp
                alert_triggered = True
                alert_message = dialog.message
                alert_timestamp = time.time()
                self.log(f"  🚨 ALERT DETECTED: {dialog.message}", "ALERT")
                
                # CRITICAL: Take screenshot BEFORE dismissing the alert
                screenshot_path = self._take_screenshot_with_popup_visible(param_name, payload)
                ...
                if screenshot_path:
                    self._store_screenshot_path(screenshot_path)
            ...
            if alert_triggered and "XSS_CONFIRMED" in alert_message:
                # Get stored screenshot path
                screenshot_path = self._get_stored_screenshot_path()
    ...
    def _take_screenshot_with_popup_visible(self, param_name, payload):
        """Take screenshot while popup is visible - CRITICAL TIMING"""
        try:
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            safe_param = re.sub(r'[^\w\-_]', '_', param_name)
            
            filename = f"popup_xss_{safe_param}_{timestamp}.png"
            screenshot_path = os.path.join('screenshots', filename)
            
            # Take screenshot immediately while popup is visible
            self.current_page.screenshot(path=screenshot_path, full_page=True)
            
            # Verify screenshot was taken
            if os.path.exists(screenshot_path):
                self.log(f"  📸 Popup screenshot captured successfully", "SUCCESS")
                return screenshot_path
            else:
                self.log(f"  ❌ Popup screenshot failed", "ERROR")
                return None
            
        except Exception as e:
            self.log(f"Popup screenshot error: {str(e)}", "ERROR")
            return None
    
    def _store_screenshot_path(self, path):
        """Store screenshot path for later use"""
        self._screenshot_path = path
    
    def _get_stored_screenshot_path(self):
        """Get stored screenshot path"""
        return getattr(self, '_screenshot_path', None)
    
    def generate_perfect_report(self):
        """Generate perfect HTML report"""
        self.log("Generating perfect report...", "INFO")
        
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join('reports', f'perfect_xss_report_{timestamp}.html')
...
<html>
<head>
    <title>Perfect XSS Scanner Report - {self.target_url}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🛡️ Perfect XSS Scanner Report</h1>
...
"""
        ...
        try:
            ...
            self.log(f"Perfect report generated: {report_path}", "SUCCESS")
            return report_path
    ...
    def scan(self):
        ...
        self.log("🚀 Starting Perfect XSS Scanner v13.0", "SUCCESS")
        ...
        try:
            ...
            # Phase 2: Persistent browser validation
            self.phase2_persistent_browser_validation(recon_data)
            
            # Generate report
            if self.running:
                report_path = self.generate_perfect_report()
    ...
    def _show_final_results(self, report_path):
        ...
        if report_path:
            self.log(f"📊 Perfect Report: {report_path}", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description='Perfect XSS Scanner v13.0')
