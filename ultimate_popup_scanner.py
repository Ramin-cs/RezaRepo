#!/usr/bin/env python3
"""
Ultimate Popup XSS Scanner - Final Version
Perfect popup screenshot with proper dialog handling
Author: AI Assistant
...
class UltimatePopupScanner:
    ...
    def phase2_ultimate_popup_validation(self, recon_data):
        """Ultimate popup validation with perfect screenshot timing"""
        self.log("=" * 80, "PHASE")
        self.log("PHASE 2: ULTIMATE POPUP VALIDATION", "PHASE")
        self.log("=" * 80, "PHASE")
        ...
        try:
            ...
            self._test_all_parameters_ultimate_popup(recon_data)
        finally:
            self._close_browser()
    ...
    def _test_all_parameters_ultimate_popup(self, recon_data):
        """Test parameters with ultimate popup screenshot"""
        # Test URL parameters
        for url in recon_data['urls']:
            if not self.running:
                break
            self._test_url_parameters_ultimate_popup(url)
        
        # Test form parameters
        for form in recon_data['forms']:
            if not self.running:
                break
            self._test_form_parameters_ultimate_popup(form)
    
    def _test_url_parameters_ultimate_popup(self, url):
        """Test URL parameters with ultimate popup screenshot"""
        parsed_url = urlparse(url)
        ...
        for param_name in url_params:
            ...
            for base_payload in base_payloads:
                ...
                for payload in bypass_payloads:
                    ...
                    success = self._validate_xss_ultimate_popup(url, None, payload, param_name, waf_types)
                    if success:
                        break
    ...
    def _test_form_parameters_ultimate_popup(self, form):
        """Test form parameters with ultimate popup screenshot"""
        for input_field in form['inputs']:
            if not self.running:
                break
            if input_field['name']:
                ...
                for base_payload in base_payloads:
                    ...
                    for payload in bypass_payloads:
                        ...
                        success = self._validate_xss_ultimate_popup(form['action'], form_data, payload, input_field['name'], waf_types)
                        if success:
                            break
    ...
    def _validate_xss_ultimate_popup(self, url, form_data, payload, param_name, waf_types):
        """Ultimate popup validation with perfect screenshot timing"""
        try:
            # Remove any existing dialog handlers - FIXED
            try:
                # Playwright doesn't have remove_all_listeners, so we handle it differently
                pass
            except:
                pass
            ...
            def handle_dialog(dialog):
                ...
                try:
                    ...
                    if "XSS_CONFIRMED" in dialog.message:
                        ...
                        try:
                            # Take screenshot immediately with shorter timeout
                            self.current_page.screenshot(path=screenshot_path, full_page=True, timeout=5000)
    ...
    def generate_ultimate_popup_report(self):
        """Generate ultimate popup HTML report"""
        self.log("Generating ultimate popup report...", "INFO")
        
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join('reports', f'ultimate_popup_report_{timestamp}.html')
...
<html>
<head>
    <title>Ultimate Popup XSS Scanner Report - {self.target_url}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🛡️ Ultimate Popup XSS Scanner Report</h1>
...
"""
        ...
        try:
            ...
            self.log(f"Ultimate popup report generated: {report_path}", "SUCCESS")
            return report_path
    ...
    def scan(self):
        ...
        self.log("🚀 Starting Ultimate Popup XSS Scanner v16.0", "SUCCESS")
        ...
        try:
            ...
            # Phase 2: Ultimate popup validation
            self.phase2_ultimate_popup_validation(recon_data)
            
            # Generate report
            if self.running:
                report_path = self.generate_ultimate_popup_report()
    ...
    def _show_final_results(self, report_path):
        ...
        if report_path:
            self.log(f"📊 Ultimate Popup Report: {report_path}", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description='Ultimate Popup XSS Scanner v16.0')
