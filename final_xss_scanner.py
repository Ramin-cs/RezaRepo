#!/usr/bin/env python3
"""
Final XSS Scanner - Perfect Version
Fixed dialog handling + Perfect popup screenshot + Error handling
Author: AI Assistant
Version: 14.0 Final
"""
...
class FinalXSSScanner:
    ...
    def calculate_professional_score(self, vuln_data):
        ...
        # Screenshot confirmation bonus (10 points) - INCREASED
        ...
        # Popup screenshot bonus (5 points) - NEW
    ...
    def _handle_dialog(self, dialog):
        """Handle dialog events - FIXED VERSION"""
        try:
            ...
            if "XSS_CONFIRMED" in dialog.message:
                screenshot_path = self._take_screenshot_with_popup_immediate()
                if screenshot_path:
    ...
    def _take_screenshot_with_popup_immediate(self):
        """Take screenshot immediately when popup appears"""
        try:
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"popup_xss_{timestamp}.png"
            ...
            self.log(f"  ❌ Immediate screenshot error: {str(e)}", "ERROR")
            return None
    ...
    def _validate_xss_persistent_browser(self, url, form_data, payload, param_name, waf_types):
        ...
        try:
            ...
            if self.dialog_handled and "XSS_CONFIRMED" in self.current_alert_message:
                ...
                vuln_data = {
                    ...
                    'alert_message': self.current_alert_message,
                    'confirmed': True,
                    'waf_bypassed': len(waf_types) > 0,
                    'popup_screenshot': self.screenshot_taken
                }
                
                # If screenshot was taken, add it to vuln_data
                if self.screenshot_taken:
                    # Find the most recent screenshot
                    screenshot_files = [f for f in os.listdir('screenshots') if f.startswith('popup_xss_')]
                    if screenshot_files:
                        latest_screenshot = max(screenshot_files, key=lambda x: os.path.getctime(os.path.join('screenshots', x)))
                        vuln_data['screenshot'] = os.path.join('screenshots', latest_screenshot)
    ...
    def generate_final_report(self):
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join('reports', f'final_xss_report_{timestamp}.html')
...
<html>
<head>
    <title>Final XSS Scanner Report - {self.target_url}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🛡️ Final XSS Scanner Report</h1>
...
"""
    ...
    def scan(self):
        ...
        self.log("🚀 Starting Final XSS Scanner v14.0", "SUCCESS")
    ...
    def _show_final_results(self, report_path):
        ...
        if report_path:
            self.log(f"📊 Final Report: {report_path}", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description='Final XSS Scanner v14.0')
