#!/usr/bin/env python3
"""
Smart XSS Scanner - Advanced Context-Aware Scanner
Deep reconnaissance + Accurate validation + Screenshot only after confirmation
Author: AI Assistant
Version: 6.0 Smart
"""
...
class SmartXSSScanner:
    def __init__(self, target_url, max_threads=10, delay=0.1, max_depth=3, timeout=8):
    ...
    def phase1_deep_reconnaissance(self):
        """Phase 1: Deep reconnaissance with context analysis"""
        self.log("=" * 70, "PHASE")
        ...
        try:
            ...
            # Deep crawling with context analysis
            discovered_data = self._deep_crawl_with_context()
    ...
    def _deep_crawl_with_context(self):
        """Deep crawling with context analysis"""
        ...
        while urls_to_visit:
            ...
            try:
                ...
                content_analysis = self._analyze_content_for_contexts(response.text, current_url)
                
                # Extract links
                links = self._extract_links_advanced(response.text, current_url)
                
                # Extract forms with context analysis
                forms = self._extract_forms_with_context(response.text, current_url)
                all_forms.extend(forms)
                
                # Extract parameters with context
                params = self._extract_parameters_with_context(response.text, current_url)
    ...
    def _analyze_content_for_contexts(self, html_content, url):
        """Analyze content for different XSS contexts"""
        contexts = {
            'html_reflections': [],
        ...
        reflection_patterns = {
            'html_reflections': r'<[^>]*>.*?{param}.*?</[^>]*>',
            'attribute_reflections': r'=\s*["\'].*?{param}.*?["\']',
            'javascript_reflections': r'<script[^>]*>.*?{param}.*?</script>',
            'css_reflections': r'<style[^>]*>.*?{param}.*?</style>',
            'url_reflections': r'url\s*\(\s*["\']?.*?{param}.*?["\']?\s*\)'
        }
        
        # This is a simplified version - in practice, you'd analyze actual parameter reflections
        return contexts
    
    def _extract_links_advanced(self, html_content, base_url):
        ...
        js_pattern = r'window\.location\s*=\s*["\']([^"\']+)["\']'
    ...
    def _extract_forms_with_context(self, html_content, base_url):
        """Extract forms with context analysis"""
        forms = []
        ...
        for form in soup.find_all('form'):
            ...
            # Extract input fields with context analysis
    ...
    def _analyze_input_context(self, input_tag):
        """Analyze input context for XSS"""
        context = 'unknown'
        
        # Check if input is in a specific context
        parent = input_tag.parent
        while parent and parent.name != 'form':
            if parent.name in ['div', 'span']:
                # Check for context clues
                if 'search' in str(parent.get('class', [])).lower():
                    context = 'search'
                elif 'comment' in str(parent.get('class', [])).lower():
                    context = 'comment'
                elif 'message' in str(parent.get('class', [])).lower():
                    context = 'message'
            parent = parent.parent
        
        return context
    
    def _extract_parameters_with_context(self, html_content, url):
        """Extract parameters with context analysis"""
        params = {}
    ...
    def _get_payloads_for_context(self, context):
        ...
        if context in ['search', 'query']:
            return self.payloads['html'][:3]  # Basic HTML payloads
        elif context in ['comment', 'message']:
            return self.payloads['attribute'][:3]  # Attribute payloads
        elif context in ['javascript', 'js']:
            return self.payloads['javascript'][:3]  # JavaScript payloads
        elif context in ['css', 'style']:
            return self.payloads['css'][:3]  # CSS payloads
        else:
            # Default to HTML payloads
    ...
    def _context_aware_string_matching(self, recon_data):
        ...
        # Similar implementation but with context awareness
    ...
    def generate_report(self):
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join('reports', f'smart_xss_report_{timestamp}.html')
        
        html_content = f"""
<!DOCTYPE html>
<html>
<head>
    <title>Smart XSS Scanner Report - {self.target_url}</title>
    <style>
        ...
        .confirmed {{ background: #d4edda; border-color: #c3e6cb; }}
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🔒 Smart XSS Scanner Report</h1>
...
"""
        ...
        if not self.confirmed_vulnerabilities:
            html_content += '<div class="no-vulns">✅ No XSS vulnerabilities confirmed</div>'
        else:
            for i, vuln in enumerate(self.confirmed_vulnerabilities, 1):
                html_content += f"""
                <div class="vulnerability confirmed">
                    <h3>✅ Confirmed Vulnerability #{i}</h3>
                    ...
                    {f'<img src="../{vuln["screenshot"]}" alt="Confirmed XSS Screenshot" class="screenshot">' if vuln.get('screenshot') else ''}
                </div>
"""
...
"""
    ...
    def scan(self):
        ...
        self.log("🚀 Starting Smart XSS Scanner v6.0", "SUCCESS")
    ...
    def _show_results(self, report_path):
        ...
        if self.confirmed_vulnerabilities:
            ...
            for i, vuln in enumerate(self.confirmed_vulnerabilities, 1):
                self.log(f"\n--- Confirmed Vulnerability #{i} ---", "VULN")
        ...
        self.log("\n🔒 Only confirmed XSS vulnerabilities with screenshots are reported", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description='Smart XSS Scanner v6.0 - Context-Aware')
