#!/usr/bin/env python3
"""
Ultimate XSS Scanner - Perfect Version
Enhanced reconnaissance + Perfect validation + Alert popup screenshot + Persistent browser
Author: AI Assistant
...
class UltimateXSSScanner:
    ...
    def phase2_smart_validation(self, recon_data):
        """Smart XSS validation with persistent browser"""
        self.log("=" * 80, "PHASE")
    ...
    def _test_url_parameters_scored(self, url):
        """Test URL parameters with detailed logging"""
        parsed_url = urlparse(url)
    ...
    def _test_form_parameters_scored(self, form):
        """Test form parameters with detailed logging"""
        for input_field in form['inputs']:
            if input_field['name']:
    ...
    def _validate_xss_browser_scored(self, url, form_data, payload, param_name, waf_types):
        """Browser validation with perfect popup screenshot capture"""
        try:
            ...
            def handle_dialog(dialog):
                ...
                # Wait longer to see the popup clearly
                time.sleep(3)
                dialog.accept()
            ...
            if alert_triggered and "XSS_CONFIRMED" in alert_message:
                # Take screenshot ONLY after popup confirmation with proper timing
                screenshot_path = self._take_screenshot_with_popup_timing(page, param_name, payload, alert_timestamp)
    ...
    def _take_screenshot_with_popup_timing(self, page, param_name, payload, alert_timestamp):
        """Take screenshot with perfect popup timing"""
        try:
            ...
            if alert_timestamp:
                time_since_alert = time.time() - alert_timestamp
                if time_since_alert < 2:  # If alert was recent, wait more
                    time.sleep(2 - time_since_alert)
            
            # Additional wait to ensure popup is captured
            time.sleep(1)
            ...
            if os.path.exists(screenshot_path):
                self.log(f"  📸 Screenshot captured successfully", "SUCCESS")
            else:
                self.log(f"  ❌ Screenshot failed", "ERROR")
    ...
    def generate_ultimate_report(self):
        """Generate ultimate HTML report with perfect scoring"""
        self.log("Generating ultimate report...", "INFO")
        ...
        if not self.confirmed_vulnerabilities:
            ...
            for i, vuln in enumerate(self.confirmed_vulnerabilities, 1):
                ...
                <div class="vulnerability {vuln_class}">
                    ...
                    {f'<img src="../{vuln["screenshot"]}" alt="XSS Screenshot with Popup" class="screenshot">' if vuln.get('screenshot') else ''}
                </div>
"""
...
"""
        ...
        try:
            ...
            self.log(f"Ultimate report generated: {report_path}", "SUCCESS")
            return report_path
    ...
    def scan(self):
        ...
        self.log("🚀 Starting Ultimate XSS Scanner v12.0", "SUCCESS")
    ...
    def _show_final_results(self, report_path):
        """Show final results with ultimate scoring"""
        self.log("=" * 80, "PHASE")
        ...
        if report_path:
            self.log(f"📊 Ultimate Report: {report_path}", "SUCCESS")

def main():
    parser = argparse.ArgumentParser(description='Ultimate XSS Scanner v12.0')
