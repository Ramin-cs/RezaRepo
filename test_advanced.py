#!/usr/bin/env python3
"""
Test Advanced Router Brute Force with improved validation
"""
...
def test_advanced_brute_force(url):
    """Test advanced brute force with improved validation"""
    print(f"Testing Advanced Brute Force: {url}")
    print("=" * 60)
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python test_advanced.py <URL>")
        print("Example: python test_advanced.py http://111.220.143.231/")
        sys.exit(1)
    
    url = sys.argv[1]
    results = test_advanced_brute_force(url)
