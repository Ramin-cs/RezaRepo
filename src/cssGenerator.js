class CSSGenerator {
  ...
  generateCSS(analysisResult) {
    const { metadata, colors, layout, responsive } = analysisResult;
    
    // Generate CSS variables
    this.generateCSSVariables(colors, metadata);
    
    // Generate base styles
    const baseStyles = this.generateBaseStyles(colors, metadata);
    
    // Generate layout styles
    const layoutStyles = this.generateLayoutStyles(layout, metadata);
    
    // Generate component styles
    const componentStyles = this.generateComponentStyles(colors, layout);
    
    // Generate responsive styles
    const responsiveStyles = this.generateResponsiveStyles(responsive, layout);
    
    // Generate utility classes
    const utilityStyles = this.generateUtilityStyles(colors);
    
    // Combine all styles
    const fullCSS = this.combineStyles([
      this.cssVariables,
      baseStyles,
      layoutStyles,
      componentStyles,
...
@media (min-width: 768px) {
  ...
  generateLayoutStyles(layout, metadata) {
...
.hero-section {
    ...
    background: linear-gradient(135deg, var(--background-color) 0%, var(--muted-color) 100%);
}
...
.footer-copyright {
  ...
  generateComponentStyles(colors, layout) {
    return `
/* Component Styles */
.card {
    background-color: var(--background-color);
...
.slide-in {
  ...
  generateResponsiveStyles(responsive, layout) {
    return `
/* Responsive Design */
